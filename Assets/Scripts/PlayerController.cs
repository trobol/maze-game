...
public class PlayerController : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (!layout[(int)(position.x * levelGen.resolution), (int)(transform.position.y * levelGen.resolution)])
		{
			add.x = 0;
		}
		if (!layout[(int)(transform.position.x * levelGen.resolution), (int)(position.y * levelGen.resolution)])
		{
			add.y = 0;
		}
